...
_lr_productions = [
  ...
  ('Declarations -> <empty>','Declarations',0,'p_Declarations_empty','lp_sin.py',15),
  ('Declaration -> Type IdList','Declaration',2,'p_Declaration','lp_sin.py',19),
  ('Type -> INT','Type',1,'p_Type_int','lp_sin.py',23),
  ('Type -> STR','Type',1,'p_Type_str','lp_sin.py',27),
  ('IdList -> IdList , id','IdList',3,'p_IdList_list','lp_sin.py',31),
  ('IdList -> id','IdList',1,'p_IdList_single','lp_sin.py',35),
  ('Statements -> Statements Statement','Statements',2,'p_Statements_list','lp_sin.py',39),
  ('Statements -> Statement','Statements',1,'p_Statements_single','lp_sin.py',44),
  ('Statement -> id = Exp','Statement',3,'p_Statement_atrib','lp_sin.py',49),
  ('Statement -> PRINT ( PrintArgs )','Statement',4,'p_Statement_print','lp_sin.py',53),
  ('PrintArgs -> PrintArgs , PrintArg','PrintArgs',3,'p_PrintArgs_list','lp_sin.py',58),
  ('PrintArgs -> PrintArg','PrintArgs',1,'p_PrintArgs_single','lp_sin.py',63),
  ('PrintArg -> str','PrintArg',1,'p_PrintArg_str','lp_sin.py',68),
  ('PrintArg -> Exp','PrintArg',1,'p_PrintArg_exp','lp_sin.py',73),
  ('Exp -> Exp + Term','Exp',3,'p_Exp_ad','lp_sin.py',78),
  ('Exp -> Exp - Term','Exp',3,'p_Exp_sub','lp_sin.py',83),
  ('Exp -> Term','Exp',1,'p_Exp_term','lp_sin.py',88),
  ('Term -> Term * Factor','Term',3,'p_Term_mul','lp_sin.py',93),
  ('Term -> Term / Factor','Term',3,'p_Term_div','lp_sin.py',98),
  ('Term -> Factor','Term',1,'p_Term_factor','lp_sin.py',104),
  ('Factor -> INT ( Exp )','Factor',4,'p_Factor_int','lp_sin.py',109),
  ('Factor -> INPUT ( str )','Factor',4,'p_Factor_input','lp_sin.py',113),
  ('Factor -> id','Factor',1,'p_Factor_id','lp_sin.py',117),
  ('Factor -> num','Factor',1,'p_Factor_num','lp_sin.py',121),
  ('Statement -> WHILE ( Cond ) CondStatements','Statement',5,'p_Statement_while','lp_sin.py',126),
  ('Statement -> IF ( Cond ) CondStatements Else','Statement',6,'p_Statement_if','lp_sin.py',130),
  ('Else -> <empty>','Else',0,'p_Else_empty','lp_sin.py',134),
  ('Else -> ELSE CondStatements','Else',2,'p_Else','lp_sin.py',138),
  ('CondStatements -> Statement','CondStatements',1,'p_CondStatements_simple','lp_sin.py',142),
  ('CondStatements -> { Statements }','CondStatements',3,'p_CondStatements_compound','lp_sin.py',146),
  ('Cond -> Cond OR Cond2','Cond',3,'p_Cond_or','lp_sin.py',150),
  ('Cond -> Cond2','Cond',1,'p_Cond','lp_sin.py',154),
  ('Cond2 -> Cond2 AND Cond3','Cond2',3,'p_Cond2_and','lp_sin.py',158),
  ('Cond2 -> Cond3','Cond2',1,'p_Cond2','lp_sin.py',162),
  ('Cond3 -> NOT ExpRel','Cond3',2,'p_Cond3_not','lp_sin.py',166),
  ('Cond3 -> ExpRel','Cond3',1,'p_Cond3','lp_sin.py',170),
  ('Cond3 -> TRUE','Cond3',1,'p_Cond3_true','lp_sin.py',174),
  ('Cond3 -> FALSE','Cond3',1,'p_Cond3_false','lp_sin.py',178),
  ('ExpRel -> Exp > Exp','ExpRel',3,'p_ExpRel_gt','lp_sin.py',182),
  ('ExpRel -> Exp < Exp','ExpRel',3,'p_ExpRel_lt','lp_sin.py',186),
  ('ExpRel -> Exp GE Exp','ExpRel',3,'p_ExpRel_ge','lp_sin.py',190),
  ('ExpRel -> Exp LE Exp','ExpRel',3,'p_ExpRel_le','lp_sin.py',194),
  ('ExpRel -> Exp EQ Exp','ExpRel',3,'p_ExpRel_eq','lp_sin.py',198),
  ('ExpRel -> Exp NEQ Exp','ExpRel',3,'p_ExpRel_neq','lp_sin.py',202),
]
